...
{
    QPixmap pixmap("D:/qt/MyGame/images/test_player.png");
...
void Player::move(int dx, int dy, const QRectF &validArea)
{
    ...
    // 无敌状态视觉反馈：闪烁效果（新增）
    // 用途：让玩家直观看到无敌状态，知道此时不会受伤
    if (m_isInvincible) {
        // 每100ms切换一次透明度，产生闪烁效果
...
// updateInvincibleState: 更新无敌状态（新增）
